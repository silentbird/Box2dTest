...
namespace Billiard {
	...
	public static class SetupBalls {
		...
		public static void AddRandomBallsWithinRectangle(GameObject ballPrefabGO, int numberOfBalls, List<BilliardBall> allBalls, float minBallRadius, float maxBallRadius,
			Vector2 rectangleSize, Vector3 rectangleCenter) {
		...
		public static void AddRandomBallsWithinCircle(GameObject ballPrefabGO, int numberOfBalls, List<BilliardBall> allBalls, float minBallRadius, float maxBallRadius,
			float circleRadius) {
			for (int i = 0; i < numberOfBalls; i++) {
				GameObject newBallGO = GameObject.Instantiate(ballPrefabGO);
